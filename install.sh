...
cmake_install_with_option() {
    ...
    DEST="$SRC_DIR/$SUBDIR/$BUILD_SUBDIR"
    cd "$DEST" || (echo "$DEST : Directory does not exist" && exit 1)
    ...
    "$SUDO" make install 2>&1 | tee "$SRC_DIR/$SUBDIR.log"

    built_dirs="$built_dirs $SUBDIR"
}

install_OpenRTM-aist() {
    DEST="$SRC_DIR/OpenRTM-aist"
    cd "$DEST" || (echo "$DEST : Directory does not exist" && exit 1)
    if [ ! -e configure ]; then
	./build/autogen
    fi
    ...
    CXXFLAGS+=" -g3" LIBPATH=/usr/lib/$(/bin/uname -p)-linux-gnu ./configure --prefix="$PREFIX" --without-doxygen "$ENABLE_DEBUG"

    built_dirs="$built_dirs OpenRTM-aist"

    SUDO=$(touch "$PREFIX" || echo "sudo")
    $SUDO make install "${SAN_FLAGS[*]}" | tee "$SRC_DIR/OpenRTM-aist.log"
...
install_octomap() {
    if [ "$DIST_KIND" = "ubuntu" ] && [ "$DIST_VER" = "14.04" ] || \
       [ "$DIST_KIND" = "debian" ] && [ "$DIST_VER" != "10" ]; then
...
install_hrpsys-base() {
    ...
    cmake_install_with_option hrpsys-base -DCOMPILE_JAVA_STUFF=OFF \
        -DBUILD_KALMAN_FILTER=OFF -DBUILD_STABILIZER=OFF -DENABLE_DOXYGEN=OFF \
        "${EXTRA_OPTION[@]}" "${ROBOT_IOB_VERSION_OPTION}"
}
...
install_hrpsys-humanoid() { 
    ...
    cmake_install_with_option hrpsys-humanoid -DCOMPILE_JAVA_STUFF=OFF \
        -DENABLE_SAVEDBG="$ENABLE_SAVEDBG" "${EXTRA_OPTION[@]}"
}
...
install_choreonoid() {
    ...
    mkdir -p "$HOME/.config/Choreonoid" || echo "$HOME/.config/Choreonoid : already exists"
...
install_choreonoid-ros()
{
    ...
    if [[ -n "$ROS_DISTRO" ]]; then
      ...
      source $SETUP_BASH
      DEST="$SRC_DIR/catkin_ws_choreonoid"
      cd "$DEST" || (echo "$DEST : does not exist" && exit 1)
